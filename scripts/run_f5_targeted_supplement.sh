...
if [[ $# -lt 1 || $# -gt 2 ]]; then
    cat >&2 <<'EOF'
usage: scripts/run_f5_targeted_supplement.sh OUTPUT_ROOT [TIMEOUT_SECONDS]

Collects one frozen, targeted F5 supplement around the two weak held-out
regimes from v3: heavy low-speed boundary and strong dynamic-gust boundary.
It adds 6 condition groups x 5 protocol-valid repeats.
EOF
    exit 2
fi
...
require_instrumentation() {
    ...
    for index in 6 7 8 9 10 11; do
        if [[ -z "${values[$index]:-}" ]]; then
            echo "F5 targeted condition lacks required instrumentation topic: ${condition_json}" >&2
...
: > "${output_root}/f5_targeted_status.tsv"
failures=0

steady_points="1.1:8:0.7 1.2:8:0.6 1.1:8:0.8"
for point in ${steady_points}; do
    IFS=: read -r mass_scale va lam <<<"${point}"
    mass_slug="$(slug "${mass_scale}")"
    condition_id="f5_targeted_m${mass_slug}_wind0"
    ...
    readarray -t values < <(read_condition_values "${condition_root}/condition.json")
    require_instrumentation "${condition_root}/condition.json" "${values[@]}"
    point_root="${output_root}/${condition_id}/va$(slug "${va}")_lam$(slug "${lam}")"
    echo "=== F5 targeted: mass=${mass_scale}, Va=${va}, lambda=${lam} ==="
    set +e
    ...
    printf 'steady\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        ...
        | tee -a "${output_root}/f5_targeted_status.tsv"
    (( valid_count >= 5 )) || failures=$((failures + 1))
done

gust_condition_id="f5_targeted_dynamic_wind"
...
if [[ ! -s "${gust_root}/condition.json" ]]; then
    "${project_root}/scripts/generate_wind_qualification_world.py" \
        ...
        --model-name standard_vtol_f5_targeted_dynamic \
        --instrument-main-wings --instrument-elevator-moment
fi
readarray -t gust_values < <(read_condition_values "${gust_root}/condition.json")
require_instrumentation "${gust_root}/condition.json" "${gust_values[@]}"

gust_points="12:1.0:4 12:0.9:8 14:0.9:4"
for point in ${gust_points}; do
    IFS=: read -r va lam amplitude <<<"${point}"
    point_root="${output_root}/gust_${amplitude}/va$(slug "${va}")_lam$(slug "${lam}")"
    echo "=== F5 targeted: gust=${amplitude}, Va=${va}, lambda=${lam} ==="
    set +e
    ...
    CA_LSC_PAYLOAD_MASS_KG="${gust_values[5]}" \
    CA_LSC_WING_FORCE_LEFT_GZ_TOPIC="${gust_values[6]}" \
    CA_LSC_WING_FORCE_RIGHT_GZ_TOPIC="${gust_values[7]}" \
    CA_LSC_ELEVATOR_JOINT_GZ_TOPIC="${gust_values[8]}" \
    CA_LSC_ELEVATOR_WRENCH_GZ_TOPIC="${gust_values[9]}" \
    CA_LSC_ELEVATOR_EFFECTIVE_ANGLE_GZ_TOPIC="${gust_values[10]}" \
    CA_LSC_ELEVATOR_DITHER_SCALE_GZ_TOPIC="${gust_values[11]}" \
    CA_LSC_ELEVATOR_ID_DITHER_ENABLED=false \
    CA_LSC_WING_FORCE_GT_ENABLED=true \
    CA_LSC_MASS_SCALE=1.0 \
    ...
    F2B_VALID_TARGET=5 F2B_MAX_ATTEMPTS=8 \
        ...
        "${point_root}" "${va}" "${lam}" "${amplitude}" "${timeout_seconds}"
    status=$?
    set -e
    valid_count="$(valid_repeat_count "${point_root}")"
    printf 'gust\t1.0\t%s\t%s\t%s\t%s\t%s\n' \
        "${va}" "${lam}" "${amplitude}" "${status}" "${valid_count}" "${point_root}" \
        | tee -a "${output_root}/f5_targeted_status.tsv"
    (( valid_count >= 5 )) || failures=$((failures + 1))
done

audit_inputs=("${project_root}/data/evaluation/f3a_eta_l_validation_v2")
if [[ -d "${project_root}/data/evaluation/f5_boundary_supplement_v3" ]]; then
    audit_inputs+=("${project_root}/data/evaluation/f5_boundary_supplement_v3")
fi
audit_inputs+=("${output_root}")
...
if (( failures > 0 )); then
    echo "F5 targeted supplement finished with ${failures} cells lacking five protocol-valid telemetry runs" >&2
    exit 1
fi

echo "F5 targeted supplement complete: ${output_root}"
